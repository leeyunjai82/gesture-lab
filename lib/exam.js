...
const srcSel = { hand: WATCH, face: '', pose: '' };
...
const results = { hand: null, face: null, pose: null }; // { state, label, prob, probs }
...
function renderSrcSelects() {
  const ids = { hand: 'selHand', face: 'selFace', pose: 'selPose' };
  COND_SOURCES.forEach(src => {
    const sel = $(ids[src]);
    sel.innerHTML = '';
...
[['selHand', 'hand'], ['selFace', 'face'], ['selPose', 'pose']].forEach(([id, src]) => {
