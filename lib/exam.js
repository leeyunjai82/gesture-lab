...
//  · 그냥 보기 = 학습 없이 내장 신호를 본다 (손가락 개수·거리·사물 이름…)
...
const COND_SOURCES = ['hand', 'face', 'pose', 'object']; // 볼 수 있는 소스 (사물은 감지 전용)
...
const srcSel = { hand: WATCH, face: '', pose: '', object: WATCH };
...
const results = { hand: null, face: null, pose: null, object: null }; // { state, label, prob, probs }
...
let rr = 0;
let lastUi = 0;
let rules = [];               // { name, op:'AND'|'OR', conds:{hand,face,pose,object} }
...
async function restoreExam() {
  ...
  try {
    ...
    for (const src of COND_SOURCES) {
      ...
      if (v && v !== WATCH) {
        if (src !== 'object' && allMeta.some(m => m.name === v && m.source === src)) await setSrc(src, v);
...
// 왼쪽 셀렉트 4개: 안 봐요 / 그냥 보기 / 내 모델들
function renderSrcSelects() {
  const ids = { hand: 'selHand', face: 'selFace', pose: 'selPose', object: 'selObject' };
  COND_SOURCES.forEach(src => {
    const sel = $(ids[src]);
    sel.innerHTML = '';
...
[['selHand', 'hand'], ['selFace', 'face'], ['selPose', 'pose'], ['selObject', 'object']].forEach(([id, src]) => {
