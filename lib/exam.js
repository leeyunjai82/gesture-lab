...
import { extract, computeSignals, sigMatch, BUILTIN_SIGS, OBJECT_NAMES, objLabel } from './features.js';
...
const SRC_KO = { hand: '손', face: '얼굴', pose: '포즈', object: '사물' };
const SOURCES = ['hand', 'face', 'pose'];               // 배울 수 있는 소스 (모델 슬롯)
const COND_SOURCES = ['hand', 'face', 'pose', 'object']; // 조건으로 쓸 수 있는 소스 (사물은 감지 전용)

let mode = null;              // watch | single | combo | seq — init 에서 정한다
let allMeta = [];
let threshold = 0.7;
// 관찰하기: 켜 둘 AI 눈 (모델 없이 그냥 본다)
const watched = { hand: true, face: false, pose: false, object: true };

// 소스별 상태
let slots = { hand: null, face: null, pose: null };   // { name, model, meta }
let singleName = null;
const lms = { hand: null, face: null, pose: null, object: null };
const lmPending = { hand: null, face: null, pose: null, object: null };
const results = { hand: null, face: null, pose: null, object: null }; // { state:'none'|'idk'|'ok', label, prob, probs }
const lastRaw = { hand: null, face: null, pose: null, object: null };
// 학습 없이 읽는 내장 신호 상태 (손가락 개수·거리·방향·입/웃음·손 들기)
const sigStates = { hand: null, face: null, pose: null, object: null };
...
// ── 시험실 상태 저장 ──
// 규칙표·순서도·고른 모델·확신도를 브라우저(localStorage)에 남겨서
// 새로고침해도 만들던 것이 그대로 이어진다.
const EXAM_KEY = 'gesturelab-exam';
let restoring = false;
...
function saveExam() {
  ...
  try {
    const str = JSON.stringify({
      mode, threshold, singleName, watched,
      bg: (typeof bgfx !== 'undefined') ? bgfx.mode : 'off',
      slots: { hand: slots.hand ? slots.hand.name : '',
               face: slots.face ? slots.face.name : '',
               pose: slots.pose ? slots.pose.name : '' },
      rules, seqSteps,
    });
...
async function restoreExam() {
  ...
  try {
    ...
    if (s.watched) COND_SOURCES.forEach(src => {
      if (typeof s.watched[src] === 'boolean') watched[src] = s.watched[src];
      const b = document.querySelector('#watchBtns [data-w="' + src + '"]');
      if (b) b.classList.toggle('on', watched[src]);
    });
    if (['watch', 'single', 'combo', 'seq'].includes(s.mode)) await setMode(s.mode);
    ...
    rules = Array.isArray(s.rules) ? s.rules.map(r => ({
      ...
      op: r.op === 'OR' ? 'OR' : 'AND',
      conds: {
        hand: (r.conds && typeof r.conds.hand === 'string') ? r.conds.hand : '',
        face: (r.conds && typeof r.conds.face === 'string') ? r.conds.face : '',
        pose: (r.conds && typeof r.conds.pose === 'string') ? r.conds.pose : '',
        object: (r.conds && typeof r.conds.object === 'string') ? r.conds.object : '',
      },
    })) : [];
    seqSteps = Array.isArray(s.seqSteps)
      ...
      : [];
    if (mode === 'single') {
      if (s.singleName && allMeta.some(m => m.name === s.singleName)) await pickSingle(s.singleName);
    } else if (mode === 'combo' || mode === 'seq') {
      for (const src of SOURCES) {
        const nm = s.slots && s.slots[src];
        if (nm && allMeta.some(m => m.name === nm && m.source === src)) await setSlot(src, nm);
      }
      renderComboSelects();
      sanitizeConds();
      if (mode === 'combo') renderRules();
      else renderSeqFlow();
      syncLms();
    }
...
function usedSources() {
  const used = new Set();
  if (mode === 'combo') {
    rules.forEach(r => COND_SOURCES.forEach(src => { if (r.conds[src]) used.add(src); }));
  } else if (mode === 'seq') {
...
function detectSources() {
  if (mode === 'watch') return COND_SOURCES.filter(src => watched[src]);
  if (mode === 'single') return activeSources();
  const used = usedSources();
  activeSources().forEach(src => used.add(src));
  return COND_SOURCES.filter(src => used.has(src));
}
...
function camStop() {
  ...
  COND_SOURCES.forEach(s => { results[s] = null; lastRaw[s] = null; sigStates[s] = null; });
}
...
function sigText(src, s) {
  ...
  if (src === 'hand') {
    ...
    (s.names || []).forEach(k => parts.push(objLabel(k)));
  }
...
function loop() {
  ...
  // 만들던 규칙·순서도를 주기적으로 남긴다 (내용이 바뀌었을 때만 기록)
  if (now - lastSaveAt > 1500) { lastSaveAt = now; saveExam(); }
}

// ── 실시간 표시 ──
function renderLive() {
  if (mode === 'watch') renderWatchOut();
  else if (mode === 'single') { renderBars(); renderSingleAnswer(); }
  else if (mode === 'combo') { renderSrcStates(); evalRules(); }
  else if (mode === 'seq') { renderSrcStates(); tickSeq(); }
}

// ── 관찰하기: 켜 둔 눈이 지금 보는 것 ──
function renderWatchOut() {
  const box = $('watchRows');
  const keys = COND_SOURCES.filter(src => watched[src]);
...
// ── 규칙표 ──
function newRule() {
  return { name: '', op: 'AND', conds: { hand: '', face: '', pose: '', object: '' } };
}
...
function condOptions(src) {
  const learned = slots[src]
    ...
    : [];
  const sigs = BUILTIN_SIGS[src].map(([v, ko]) =>
    ['s:' + v, OBJECT_NAMES[v] ? objLabel(v) : T(ko)]);
  return { learned, sigs };
}
...
async function setMode(next) {
  if (mode === next) return;
  mode = next;
  const watch = next === 'watch', single = next === 'single';
  $('modeWatch').classList.toggle('on', watch);
  $('modeSingle').classList.toggle('on', single);
  $('modeCombo').classList.toggle('on', next === 'combo');
  $('modeSeq').classList.toggle('on', next === 'seq');
  $('modeHint').textContent = watch ? T('카메라만 켜면 바로 보여요')
    : single ? T('배운 모델 1개를 시험해요')
    : next === 'combo' ? T('모델을 묶어 규칙을 만들어요')
    : T('동작을 순서대로 이어 봐요');
  $('pickTitle').textContent = watch ? T('무엇을 볼까요?') : T('모델 고르기');
  $('ansTitle').textContent = watch ? T('보이는 것') : T('AI의 답');
  $('watchPick').style.display = watch ? '' : 'none';
  $('sglPick').style.display = single ? '' : 'none';
  $('cmbPick').style.display = (watch || single) ? 'none' : '';
  $('thrBox').style.display = watch ? 'none' : '';
  $('answer').style.display = watch ? 'none' : '';
  $('watchOut').style.display = watch ? '' : 'none';
...
document.querySelectorAll('#watchBtns .db').forEach(b => {
  b.addEventListener('click', () => {
    const src = b.dataset.w;
    watched[src] = !watched[src];
    b.classList.toggle('on', watched[src]);
    syncLms();
  });
});
...
(async function init() {
  ...
  if (mode === null) await setMode('watch');   // 처음이면 관찰하기부터
  $('engine').textContent = T('준비 완료');
