...
// ── 출력 장치 (임베디드 제어처럼: LED + 말하기) ──
// 답이 나오면 LED 색을 켜고 정한 문장을 소리 내어 읽는다 (speechSynthesis — 브라우저 내장).
const LED_COLORS = {
  red: ['빨강', '#FF5147'], yellow: ['노랑', '#FFD23E'], green: ['초록', '#35CE68'],
  blue: ['파랑', '#3D8BFF'], purple: ['보라', '#B36BFF'], cyan: ['청록', '#00BEDC'],
  white: ['하양', '#FFFFFF'],
};
let sglReactions = {};            // 클래스 이름 → { led:'', say:'' }
let lastKey = null, pendKey = null, pendCount = 0;

function setLed(hex) {
  const el = $('ledLamp');
  el.classList.toggle('on', !!hex);
  el.style.background = hex || '';
  el.style.boxShadow = hex ? '0 0 10px 2px ' + hex : '';
}
...
function speak(text) {
  ...
  try {
    ...
    u.lang = (typeof GL_LANG !== 'undefined' && GL_LANG === 'en') ? 'en-US' : 'ko-KR';
    u.onstart = () => $('spkIcon').classList.add('on');
    u.onend = () => $('spkIcon').classList.remove('on');
    u.onerror = () => $('spkIcon').classList.remove('on');
    speechSynthesis.speak(u);
  } catch (e) { /* 무시 */ }
}

// 결과가 프레임마다 깜빡이지 않게, 같은 답이 2번 이어질 때만 반응을 바꾼다.
function reactTo(key, reaction) {
  if (key === pendKey) pendCount++;
  else { pendKey = key; pendCount = 0; return; }
  if (pendCount < 2 || key === lastKey) return;
  lastKey = key;
  if (!key || !reaction) { setLed(null); return; }
  setLed(reaction.led ? LED_COLORS[reaction.led][1] : null);
  if (reaction.say && reaction.say.trim()) speak(reaction.say.trim());
}

function ledSelect(value, onChange) {
  const sel = document.createElement('select');
  sel.className = 'inp';
  const off = document.createElement('option');
  off.value = ''; off.textContent = T('불 끄기');
  sel.appendChild(off);
  Object.keys(LED_COLORS).forEach(k => {
    const o = document.createElement('option');
    o.value = k; o.textContent = T(LED_COLORS[k][0]);
    sel.appendChild(o);
  });
  sel.value = value || '';
...
function renderSglReactions() {
  const slot = slots.hand || slots.face || slots.pose;
  const sec = $('sglReactSec');
  const box = $('sglReact');
  box.innerHTML = '';
  if (!slot) { sec.style.display = 'none'; return; }
  sec.style.display = '';
  (slot.meta.classes || []).forEach(cls => {
    if (!sglReactions[cls]) sglReactions[cls] = { led: '', say: '' };
    const r = sglReactions[cls];
    const row = document.createElement('div');
    row.className = 'react-row';
    const nm = document.createElement('span');
    nm.className = 'rn'; nm.textContent = cls; nm.title = cls;
    row.appendChild(nm);
    row.appendChild(ledSelect(r.led, v => { r.led = v; }));
    const say = document.createElement('input');
    say.className = 'inp'; say.maxLength = 40;
    say.placeholder = T('말할 말 (예: 안녕!)');
    say.value = r.say;
    say.addEventListener('input', () => { r.say = say.value; });
    row.appendChild(say);
    box.appendChild(row);
  });
}
...
function renderSingleAnswer() {
  ...
  if (!slot) { el.textContent = T('모델을 골라 주세요'); el.classList.add('idk'); reactTo(null); return; }
  if (!camReady()) { el.textContent = T('카메라를 켜 주세요'); el.classList.add('idk'); reactTo(null); return; }
  const st = results[slot.meta.source];
  const w = stateWord(st);
  el.textContent = w.txt;
  if (w.cls === 'ok') {
    el.classList.add('sure');
    reactTo('c:' + st.label, sglReactions[st.label]);
  } else {
...
// ── 규칙표 ──
function newRule() {
  return { name: '', op: 'AND', conds: { hand: '', face: '', pose: '' }, led: '', say: '' };
}
...
function renderRules() {
  ...
  rules.forEach((rule, ri) => {
    ...
    // 반응 (LED + 말하기) — 임베디드 장치를 제어하듯 출력을 정한다
    const react = document.createElement('div');
    react.className = 'react-row';
    const rlb = document.createElement('span');
    rlb.className = 'rn'; rlb.style.width = '38px';
    rlb.textContent = T('반응');
    react.appendChild(rlb);
    react.appendChild(ledSelect(rule.led, v => { rule.led = v; }));
    const say = document.createElement('input');
    say.className = 'inp'; say.maxLength = 40;
    say.placeholder = T('말할 말 (예: 발사!)');
    say.value = rule.say;
    say.addEventListener('input', () => { rule.say = say.value; });
    react.appendChild(say);
...
function evalRules() {
  ...
  const active = SOURCES.filter(src => slots[src]);
  el.classList.remove('sure', 'idk');
  if (!active.length) { el.textContent = T('모델을 골라 주세요'); el.classList.add('idk'); markFired(-1); reactTo(null); return; }
  if (!camReady()) { el.textContent = T('카메라를 켜 주세요'); el.classList.add('idk'); markFired(-1); reactTo(null); return; }
  if (!rules.length) { el.textContent = T('규칙을 더해 보세요'); el.classList.add('idk'); markFired(-1); reactTo(null); return; }
...
async function setMode(next) {
  ...
  sglReactions = {};
  lastKey = null; setLed(null);
  if (single) { renderSingleList(); renderSglReactions(); }
