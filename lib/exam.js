...
const SEQ_MAX = 8;
let seqSteps = [];            // [{ src, cls }]  cls 는 클래스 이름 또는 '__none__'
...
// 고를 수 있는 조건: 켜진 소스마다 [종류들, 안 보여요]
function seqOptions() {
  const opts = [];
  SOURCES.filter(src => slots[src]).forEach(src => {
    (slots[src].meta.classes || []).forEach(cls => {
      opts.push([src + '|' + cls, T(SRC_KO[src]) + ': ' + cls]);
    });
    opts.push([src + '|__none__', T(SRC_KO[src]) + ': ' + T('안 보여요')]);
  });
  return opts;
}

function seqCondMet(step) {
  const st = results[step.src];
  if (step.cls === '__none__') return !!st && st.state === 'none';
  return !!st && st.state === 'ok' && st.label === step.cls;
}
...
function renderSeqFlow() {
  ...
  seqSteps.forEach((step, i) => {
    ...
    if (sel.value && sel.value !== cur) {
      const [src, cls] = sel.value.split('|');
      step.src = src; step.cls = cls;
    }
...
function tickSeq() {
  ...
  if (!active.length) msg = T('모델을 골라 주세요');
...
$('seqAdd').addEventListener('click', () => {
  const opts = seqOptions();
  if (!opts.length) { toast(T('먼저 모델을 골라 주세요')); return; }
  if (seqSteps.length >= SEQ_MAX) { toast(T('단계는 8개까지예요')); return; }
  const [src, cls] = opts[0][0].split('|');
  seqSteps.push({ src, cls });
  seqRun(false);
  renderSeqFlow();
});
...
[['slotHand', 'hand'], ['slotFace', 'face'], ['slotPose', 'pose']].forEach(([id, src]) => {
  $(id).addEventListener('change', async e => {
    ...
    if (mode === 'seq') {
      // 꺼진 소스를 쓰던 단계는 지운다
      seqSteps = seqSteps.filter(s => slots[s.src]);
      seqRun(false);
      renderSeqFlow();
    } else renderRules();
  });
