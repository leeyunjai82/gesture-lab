...
export const BUILTIN_SIGS = {
  ...
  // 사물: 전부 학습 없이 되는 것 (배우기는 없다 — 감지 전용 소스)
  object: [],   // 아래 OBJECT_NAMES 로 채운다
};

// 사물 감지에서 아이들에게 보여줄 것만 고른다 (COCO 라벨 → [한국어, 영어])
// 주의: 이 이름들은 i18n 사전에 넣지 않는다 — '가위' 같은 흔한 말이 사전에 있으면
// 학생이 지은 종류 이름까지 번역돼 버린다. objLabel() 로만 표시한다.
export const OBJECT_NAMES = {
  person: ['사람', 'Person'], cup: ['컵', 'Cup'], bottle: ['병', 'Bottle'],
  book: ['책', 'Book'], scissors: ['가위', 'Scissors'],
  'cell phone': ['휴대폰', 'Phone'], 'teddy bear': ['인형', 'Teddy bear'],
  'sports ball': ['공', 'Ball'], banana: ['바나나', 'Banana'],
  apple: ['사과', 'Apple'], orange: ['오렌지', 'Orange'], bowl: ['그릇', 'Bowl'],
  spoon: ['숟가락', 'Spoon'], fork: ['포크', 'Fork'], chair: ['의자', 'Chair'],
  backpack: ['가방', 'Bag'], umbrella: ['우산', 'Umbrella'], clock: ['시계', 'Clock'],
  keyboard: ['키보드', 'Keyboard'], mouse: ['마우스', 'Mouse'],
  remote: ['리모컨', 'Remote'], laptop: ['노트북', 'Laptop'],
  tv: ['텔레비전', 'TV'], toothbrush: ['칫솔', 'Toothbrush'],
};

export function objLabel(token) {
  const e = OBJECT_NAMES[token];
  if (!e) return token;
  const en = (typeof GL_LANG !== 'undefined' && GL_LANG === 'en');
  return en ? e[1] : e[0];
}

BUILTIN_SIGS.object = [['hide', '아무것도 안 보여요']]
  .concat(Object.keys(OBJECT_NAMES).map(k => [k, k]));   // 라벨은 objLabel 로 채운다
...
export function computeSignals(source, result, vw, vh) {
  ...
  if (source === 'object') return objectSignals(result);
  return null;
}

function objectSignals(result) {
  const dets = result && result.detections;
  if (!dets || !dets.length) return { shown: false, names: [] };
  const names = [];
  dets.forEach(d => {
    const c = d.categories && d.categories[0];
    if (c && OBJECT_NAMES[c.categoryName] && !names.includes(c.categoryName)) {
      names.push(c.categoryName);
    }
  });
  return { shown: names.length > 0, names };
}
...
export function sigMatch(source, want, s) {
  ...
  if (source === 'object') {
    return Array.isArray(s.names) && s.names.includes(want);
  }
  return false;
}
