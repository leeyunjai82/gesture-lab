...
import {
  ...
  ObjectDetector, ImageSegmenter, DrawingUtils,
...
const SOURCES = {
  ...
  object: {
    // 일상 사물 감지 (학습 없이) — EfficientDet-Lite0
    model: './models/efficientdet_lite0.tflite',
    make: (vision, base) => ObjectDetector.createFromOptions(vision, {
      baseOptions: base, runningMode: 'VIDEO', scoreThreshold: 0.5, maxResults: 4,
    }),
  },
};
...
export function drawResult(ctx, source, result) {
  ...
  if (source === 'hand') {
    ...
    (result.landmarks || []).forEach(lm => {
      du.drawConnectors(lm, PoseLandmarker.POSE_CONNECTIONS, { color: ACC, lineWidth: 3 });
      du.drawLandmarks(lm, { color: DOT, fillColor: DOT, lineWidth: 1, radius: 3 });
    });
  } else if (source === 'object') {
    // 상자만 그린다 — 캔버스가 CSS 로 좌우 반전되므로 글자를 쓰면 뒤집혀 보인다.
    // 사물 이름은 화면 아래 상태 줄이 담당한다.
